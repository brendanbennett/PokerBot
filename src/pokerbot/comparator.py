...
def firstHandIsBetter(h1, h2): # given two hands, with the 5 cards + rank + relevant kicker details, say which wins
    ...
    if h1[5]==5 or h1[5]==0: # flush or high card: check all five cards
        for i in range(5):
            if h1[4 - i] != h2[4 - i]: return h1[4 - i] > h2[4 - i]
        return None # chop
    for i in range(6,10):
        if h1[i] != h2[i]: return h1[i] > h2[i] # one is higher, so that one wins
        if len(h1) == i+1: return None # all were the same, so it's a chop
...
def compare_raw(cards_input): # keep letting the user enter hands as long as they want to
    #cards = input("Enter the cards (board, then 2 to 10 hands). Sample format: 5c6dTh4dJd KhQh AsJs 7h9d ...>")
    cards = cards_input
    lenCards = len(cards)
    ...
    if len(choppers) > 0:
        ...
        print(
            "The following players chop: " + str(winner + 1) + ", " + ", ".join(str(x) for x in chopNums) + " (all have " +
            ranks[allHands[winner][5]] + ")")
    else:
        print('Player %s wins, with ' % str(winner + 1) + ranks[allHands[winner][5]])

    for i in range(len(allHands)):
        pH = allHands[i]
        for j in range(len(pH)): pH[j] -= 48 if j != 5 else -51
